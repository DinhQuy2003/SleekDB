...
class Store
{
   ...
   */
  public function __construct($storeName, $dataPath, $replicaNodes = [], $shardNodes = []) {
    $this->dataPath = rtrim($dataPath, '/') . '/' . $storeName . '/';
  ...
  public function insert(array $data) {
    ...
    // Gửi dữ liệu đến các node replica
...
//        throw new InvalidArgumentException("Documents have to have the primary key \"$primaryKey\". Got data: $documentString");
  ...
  private function replicateData(array $data) {
    foreach ($this->replicaNodes as $node) {
        $url = $node . "/replicate.php";
  ...
  public function getShardNode($shardKey) {
    $index = crc32($shardKey) % count($this->shardNodes);
    return $this->shardNodes[$index];
  }
}
